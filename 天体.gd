...
func 取轨道速率(全局位置:Vector3) -> Vector3:
	var d := 全局位置 - self.global_position
...
# 获取本天体的公转周期
func 取公转周期() -> float:
	return 2.0 * PI * sqrt(pow(轨道半径, 3) / 父天体.天体质量)
...
func _physics_process(delta):
	if (父天体 != null):
		...
		constant_linear_velocity += 重力加速度 * delta
		global_position += constant_linear_velocity * delta
	
	global_position += constant_angular_velocity * delta
